  ...
  const fetchData = async () => {
    try {
      ...
      if (response.data) {
        ...
        const sortedCinemas = response.data.data.sort((a, b) => a.id - b.id);
      ...
      console.error("Lỗi khi tải danh sách rạp chiếu:", err);
      setError(
        err.message || "Không thể tải danh sách rạp chiếu. Vui lòng thử lại sau."
      );
    } finally {
      setLoading(false);
    }
    ...
    <div className="px-4 py-6 sm:px-6, lg:px-8 animate-fadeIn">
      ...
      {cinemas.length === 0 ? (
        <div className="text-center py-12 bg-white dark:bg-background-paper-dark rounded-lg shadow-card">
          ...
          <p className="mt-1 text-sm text-text-secondary dark:text-text-secondary-dark">
            Không có rạp chiếu nào phù hợp với tiêu chí tìm kiếm của bạn.
          </p>
        </div>
      ) : (
        <>
