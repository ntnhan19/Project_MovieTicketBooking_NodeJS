...
const GenreList = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await genreService.getList({
      ...
      setError(
        err.message || "Không thể tải danh sách thể loại. Vui lòng thử lại sau."
      );
    } finally {
      setLoading(false);
    }
    ...
    <div className="px-4 py-6 sm:px-6 lg:px-8 animate-fadeIn">
      <div className="sm:flex sm:items-center sm:justify-between">
        <div>
          ...
          </h1>          <p className="mt-1 text-sm text-text-secondary dark:text-text-secondary-dark">
            Tổng số: {totalItems} thể loại
          </p>
        </div>
      ...
      {genres.length === 0 ? (
        <div className="text-center py-12 bg-white dark:bg-background-paper-dark rounded-lg shadow-card">
          ...
          <p className="mt-1 text-sm text-text-secondary dark:text-text-secondary-dark">
            Không có thể loại nào phù hợp với tiêu chí tìm kiếm của bạn.
          </p>
        </div>
      ) : (
        <>
